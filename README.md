...
- **Segment Writer**：每块按 7 步结构独立编写
- **Verifier**：检查覆盖完整度、7 步结构、公式准确性

### 7 步模块化输出

Skill 生成的每节包含完整的 7 步结构：
...
Skill 会在必要时自动用 tikz（LaTeX）生成辅助理解的示意图，包括知识结构图、对比图、流程图、几何示意图等，每章至少评估一次是否需要配图。
